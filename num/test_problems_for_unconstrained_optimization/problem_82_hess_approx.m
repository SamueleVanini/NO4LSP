function Hess = problem_82_hess_approx(x_bar, h, specific, gradF, gradF_xbar)
    %PROBLEM_82_HESS_APPROX Approximation of the Jacobian of the Problem 82
    %function
    ...
    %           logical value
    %       gradF - gradient of the Problem 82 function
    %           function handle
    ...
    e1(1:3:n) = h_vec(1:3:n);
    e2(2:3:n) = h_vec(2:3:n);
    e3(3:3:n) = h_vec(3:3:n);

    % Compute hessian component
    approx = @(e) (gradF(x_bar + e) - gradF_xbar)./h_vec;
    eval = [approx(e1), approx(e2), approx(e3)];

    % Hessian indices for sparsity
    row_idx = (2:n)';
    col_idx = mod(row_idx - 1, 3) + 1;
