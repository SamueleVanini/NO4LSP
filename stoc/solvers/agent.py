...
class Agent:
    ...
    def __init__(self, discount: float, model: AirlineModel, customersParams: dict[str, Any]):
    ...
    def _init_tables(self):
        ...
        for price_bin in range(self.model.nbins_price):
            ...
            for s in range(self.model.tot_tickets + 1):
                ...
                # min needed since we can't sell more tickets than the number of customers
                for k in range(0, min(s, ncustomers) + 1):
                ...
                # normalize probabilities forcing the sum to be 1
        ...
        # we want to push the algorithm to try to sell all the tickets or if not possible sell them at a higher price
        for s in range(self.model.tot_tickets + 1):
    ...
    def solve(self):
        for days_left in tqdm(range(1, self.model.horizon + 1)):
            for t in range(self.model.tot_tickets + 1):
                future_rewards = self.T[:, :, t] @ self.V[:, :, days_left - 1]
                for b in range(self.model.nbins_price):
                    current_reward = self.R[t, :]
                    potential_reward = self.discount * future_rewards[b, :]
    ...
    def get_policy(self) -> dict[tuple[int, int, int], int]:
        ...
        for day in range(self.model.horizon, 0, -1):
            for ticket_left in range(self.model.tot_tickets + 1):
                future_rewards = self.T[:, :, ticket_left] @ self.V[:, :, day - 1]
                for price_bin in range(self.model.nbins_price):
                    current_reward = self.R[ticket_left, :]
                    potential_reward = self.discount * future_rewards[price_bin, :]
                    tot_rewards = current_reward + potential_reward
